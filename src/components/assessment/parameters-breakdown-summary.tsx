...
import { ParametersBreakdown } from "@/lib/api";
...
interface ParametersBreakdownSummaryProps {
    data: ParametersBreakdown | null;
    isLoading?: boolean;
}

export function ParametersBreakdownSummary({ data, isLoading }: ParametersBreakdownSummaryProps) {
    ...
    const { total_parameters, overall_complexity, stats, parameters } = data;
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    Total parameters: {total_parameters}. Per-parameter: name, type, dashboards/reports containing.
                </CardDescription>
            </CardHeader>
            <CardContent>
                {statItems.length > 0 && (
                ...
                <Collapsible open={open} onOpenChange={setOpen}>
                    <CollapsibleTrigger className="flex items-center gap-2 rounded-md py-2 text-left text-sm font-medium text-muted-foreground transition-colors hover:text-foreground">
                        ...
                        Per-parameter breakdown ({parameters.length} {parameters.length === 1 ? "parameter" : "parameters"})
                    </CollapsibleTrigger>
                    <CollapsibleContent>
                            ...
                            <Table>
                                <TableHeader>
                                    <TableRow>
                                        ...
                                        <TableHead>Variable type</TableHead>
